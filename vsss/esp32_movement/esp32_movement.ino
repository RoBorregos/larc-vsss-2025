...
void VelocityTracker()
{
  ...
  //to then be added to the current PWM limiting the pwm range to -255 < pwm < 255
    ...
    actual_pwm.first = abs(actual_pwm.first) > 254  ? 254 * (signbit(actual_pwm.first) ? -1 : 1): actual_pwm.first; 
    actual_pwm.second = abs(actual_pwm.second) > 254  ? 254 * (signbit(actual_pwm.second) ? -1 : 1) : actual_pwm.second; 
